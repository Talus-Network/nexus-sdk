...
- `tool inspect-onchain` command that derives `Tool` and `ToolGas` object IDs from an FQN, probes both on-chain, and decodes the `Tool` object's `package_address`, `module_name`, `tool_witness_id`, schemas, `workflow_authorization_cap_first`, and registration timestamps into stable JSON.
...
- `tap execute --grant-bind <vertex>:<state>:<package>::<module>::<function>` flag (repeatable). The CLI parses each binding, resolves the state object's ref, and forwards them as `AgentDagExecuteOptions::grant_binds`. The execute PTB then mints a `WorkflowVertexAuthorizationGrant` for each vertex and calls the named Move function with `(state, vertex_bytes, grant_id)` before the leader dispatches the walk, so cap-gated TAP skills can drain into a state slot whose authorized grant id is set atomically with the execution start.
...
- `TapActions::inspect_endpoint` returning an `EndpointInspection` with the on-chain object ref, originating package id, the resolved active `TapEndpointRecord`, and the full list of registry-stored revisions bound to the endpoint object.
- `TapActions::bind_agent_skill` composed PTB that runs `tap::create_agent` and `tap::register_skill` in a single transaction, with `BindAgentSkillParams`/`BindAgentSkillResult` capturing operator, artifact, endpoint override, agent/skill ids, agent object ref, endpoint object ref, and the derived config-digest plus its `TapConfigDigestInput`.
...
- `transactions::dag::VertexGrantBind` + `MoveCallTarget` for chaining a `dag::create_vertex_authorization_grant` mint and a custom tap-package `bind_pending_grant`-style Move call inside the same execute PTB. `AgentDagExecuteInput::grant_binds` and `AgentDagExecuteOptions::grant_binds` thread the list through; `transactions::dag::execute_agent_dag` consumes them between `lock_payment_state_for_tools` and `request_network_to_execute_walks` so the leader sees a fully-armed execution (grant attached, state-side slot populated) before dispatching. This makes the production-correct cap-gated transfer pattern from the TAP development guide reachable through a single `nexus tap execute` call.
