...
- `idents::generated`, address-free `ModuleAndNameIdent` constants rendered at build time (via `build.rs`) from committed normalized Move package IR under `src/idents/generated/ir/*.json`, giving a single regeneratable source of truth for on-chain identifiers. The `generate_binding` integration test refreshes the IR from a localnet over gRPC (`just sdk regenerate-idents`); generated constants stay package-id-agnostic so the runtime-injected `NexusObjects` still supplies the package address at call time.
