...
- The `idents` submodules (`workflow`, `primitives`, `registry`, `scheduler`, `tap`, `move_std`, `sui_framework`) now derive their `ModuleAndNameIdent` constants from committed normalized Move package IR under `src/idents/generated/ir/*.json`, rendered at build time by `build.rs`, giving a single regeneratable source of truth for on-chain identifiers; hand-written argument/`TypeTag` helpers are retained on top. Generated constants are package-id-agnostic, so the runtime-injected `NexusObjects` still supplies the package address at call time. The `generate_binding` binary refreshes the IR from a localnet over gRPC (`cargo run --features binding_codegen --bin generate_binding`, wrapped by `just sdk regenerate-idents`).
- `ScheduledAgentExecutionConfig::resolve_dag(crawler, objects)`, mirroring Move's `agent_registry::resolve_agent_execution_config_dag`. For default-executor and runtime-selected skills the dag is encoded in the config; for pinned skills it is resolved from the skill record's `dag_binding`. The leader uses it to recover the dag at occurrence-execution time.
- `CreateTaskTapPayment::selected_dag()` and `AgentTaskPayment::selected_dag()` accessors covering both enum variants in one call.
...
- **Breaking**: `transactions::tap::new_agent_funded_task`, `new_invoker_funded_agent_task`, and `transactions::scheduler::new_agent_execution_policy` no longer take a `dag_id` argument; pass `selected_dag: Option<Address>` only (the policy now keeps `selected_dag = None` for pinned skills). `nexus::tap::CreateAgentTaskParams` drops its `dag_id` field for the same reason.
...
- Scheduled TAP tasks now match Move's `agent_registry::resolve_agent_execution_config_dag` contract: `transactions::tap::scheduled_agent_execution_config_arg` and `transactions::scheduler::new_agent_execution_policy` pass `selected_dag` straight through from the caller, so pinned skills store `selected_dag = None` (avoiding `EDagSelectionRedundant` at occurrence execution) and runtime-selected skills require `Some(_)`. CLI `tap schedule-task` derives the right value from the skill's on-chain `dag_binding`.
- `nexus_sdk::events::parsing::allows_foreign_emitter` now allow-lists `RequestScheduledOccurrenceEvent` so agent-funded scheduled occurrences triggered through a user package (e.g. via `scheduler::add_occurrence_*_for_agent_funded_task`) decode instead of being silently dropped by the leader's event poller.
