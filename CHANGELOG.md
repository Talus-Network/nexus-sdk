...
- `tap payments resolve --execution-id <OBJECT_ID>` command that wraps the on-chain `nexus_workflow::dag::accomplish_tap_execution_payment` PTB so the holder of the shared `DAGExecution` object can settle its standard TAP payment directly. Backed by `TapActions::accomplish_execution_payment` and `AccomplishExecutionPaymentParams`/`AccomplishExecutionPaymentResult` on the SDK side.
