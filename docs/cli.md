...
If the `--batch` flag is passed, the command will accept a URL of a webserver hosting multiple tools and register all of them at once. `nexus-toolkit` automatically generates a `GET /tools` endpoints that returns a list of URLs of all tools registered on that server. The CLI will then iterate over the list and register each tool.
